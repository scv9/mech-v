<?php
class ModelSalePhotos extends Model
{
    ...
    public function editPhotos($v_id,$date)
    {
    ...
    public function deletePhoto($photo_id) {
       # $this->db->query("UPDATE " . DB_PREFIX . "customer_vehicle_photo SET deleted = 1, deleted_at = now() WHERE id = '" . (int)$photo_id . "'");
        $this->db->query("DELETE FROM " . DB_PREFIX . "customer_vehicle_photo WHERE id = '" . (int)$photo_id . "'");
    }
    ...
    public function getCustomers($data = array()) {
        $sql = "SELECT DISTINCT c.id, cgd.name AS customer_group FROM " . DB_PREFIX . "customer c LEFT JOIN " . DB_PREFIX . "customer_group_description cgd ON (c.customer_group_id = cgd.customer_group_id) LEFT JOIN " . DB_PREFIX . "customer_vehicle ocv ON (c.customer_id = ocv.c_id) LEFT JOIN " . DB_PREFIX . "customer_vehicle_photo ocvp ON (cv.vehicle_num = ocvp.v_id) WHERE cgd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
        ...
        if (!empty($data['filter_email'])) {
            $implode[] = "ocv.vehicle_num LIKE '" . $this->db->escape($data['filter_email']) . "%'";
        }
       ...
       /* if (isset($data['filter_newsletter']) && !is_null($data['filter_newsletter'])) {
        ...
        if (!empty($data['filter_date_added'])) {
            $implode[] = "DATE(ocvp.created_at) = DATE('" . $this->db->escape($data['filter_date_added']) . "')";
        }
    ...
    public function getCustomersReport($data = array()) {
        $sql = "SELECT DISTINCT ocvp.c_id, ocvp.added_by, CONCAT(oc.firstname, ' ', oc.lastname) AS c_name, ocvp.v_id, DATE(ocvp.created_at) AS date_created FROM oc_customer_vehicle_photo ocvp JOIN oc_customer_vehicle ocv ON (ocvp.v_id = ocv.vehicle_num) JOIN oc_customer oc ON (oc.customer_id = ocvp.c_id)" ;
        ...
        if (!empty($data['filter_email'])) {
            $implode[] = "ocvp.v_id LIKE '" . $this->db->escape($data['filter_email']) . "%'";
        }
    ...
    public function getCustomerPhotos($c_id,$v_id,$p_date,$added_by){
        $sql = "SELECT ocvp.id, ocvp.deleted, ocvp.photo, ocvp.caption, DATE_FORMAT(ocvp.created_at, '%H:%i') as time_created FROM oc_customer_vehicle_photo ocvp WHERE ocvp.v_id = '".$v_id."' AND DATE_FORMAT(ocvp.created_at,'%Y-%m-%d') LIKE '%".$p_date."%' AND ocvp.c_id=$c_id and ocvp.added_by = '".$added_by."'";
        $query = $this->db->query($sql);
        return $query->rows;
    }
    ...
    public function getTotalCustomersReport($data = array()) {
        $sql = "SELECT COUNT(*) AS total FROM (SELECT DISTINCT ocvp.c_id, ocvp.added_by, CONCAT(oc.firstname, ' ', oc.lastname) AS c_name, ocvp.v_id as v_id, DATE(ocvp.created_at) AS date_created FROM oc_customer_vehicle_photo ocvp JOIN oc_customer_vehicle ocv ON (ocvp.v_id = ocv.vehicle_num) JOIN oc_customer oc ON (oc.customer_id = ocvp.c_id)) AS total";

        $implode = array();

        if (!empty($data['filter_name'])) {
            $implode[] = "c_name LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
        }

        if (!empty($data['filter_email'])) {
            $implode[] = "v_id LIKE '" . $this->db->escape($data['filter_email']) . "%'";
        }
/*
        if (isset($data['filter_newsletter']) && !is_null($data['filter_newsletter'])) {
...
*/
        if (!empty($data['filter_date_added'])) {
            $implode[] = "date_created = DATE('" . $this->db->escape($data['filter_date_added']) . "')";
        }
