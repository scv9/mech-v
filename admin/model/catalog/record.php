...
class ModelCatalogRecord extends Model
{
	...
	public function getRecords($data = array())
	{
		...
		if ($data) {
			...
			$record_data = $this->cache->get($mark_record.'.' . (int) $this->config->get('config_language_id'));
			if (!$record_data) {
				...
				$this->cache->set($mark_record.'.' . (int) $this->config->get('config_language_id'), $record_data);
			}
			return $record_data;
		}
	}
