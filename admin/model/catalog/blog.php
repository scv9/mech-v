...
class ModelCatalogBlog extends Model
{
	...
	{
		$blog_data = $this->cache->get($mark_category.'.' . (int) $this->config->get('config_language_id') . '.' . (int) $parent_id);
		if (!$blog_data) {
			$blog_data = array();
			...
			$this->cache->set($mark_category.'.' . (int) $this->config->get('config_language_id') . '.' . (int) $parent_id, $blog_data);
		}
		return $blog_data;
	}
