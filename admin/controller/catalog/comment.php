...
class ControllerCatalogComment extends Controller
{
	...
	private function getList()
	{
		...
		foreach ($results as $result) {
			...
			$this->data['karma']      = $this->model_catalog_treecomments->getRatesByCommentId($result[$this->data['mark_name'].'_id'], $this->data['mark'], true);
			$this->data['karma_all']  = $this->model_catalog_treecomments->getRatesByCommentId($result[$this->data['mark_name'].'_id'], $this->data['mark']);
	...
	private function getForm()
	{
       ...
       $this->data['agoo_menu'] = $this->controller_agooa_adminmenu->index();
		...
		$this->data['karma']     = $this->model_catalog_treecomments->getRatesByCommentId($comment_id, $this->data['mark'], true);
        $this->load->model('sale/customer');

   		foreach ($this->data['karma'] as $num =>$karma_val) {
			$results_karama = $this->model_sale_customer->getCustomer($karma_val['customer_id']);
		...
		$this->data['karma_all'] = $this->model_catalog_treecomments->getRatesByCommentId($comment_id, $this->data['mark']);
	...
	public function insert()
	{
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (SCP_VERSION < 2) {
				//$this->redirect($this->url->link('catalog/comment', 'token=' . $this->session->data['token'], 'SSL'));
			} else {
				//$this->response->redirect($this->url->link('catalog/comment', 'token=' . $this->session->data['token'], 'SSL'));
			}
		}
		$this->getForm();
	}
