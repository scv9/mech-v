<?php
class ControllerSalePhotos extends Controller
{
    private $error = array();

    public function index() {
        $this->load->language('sale/photos');

        $this->document->setTitle($this->language->get('heading_title'));

        $this->load->model('sale/photos');
    ...
    public function edit() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            $this->model_sale_photos->editPhotos($this->request->get['vehicle_id'], $this->request->post);
            ...
            $this->response->redirect($this->url->link('sale/photos', 'token=' . $this->session->data['token'] . $url, 'SSL'));
        }
    ...
    public function delete() {
        ...
        if (isset($this->request->post['selected']) && $this->validateDelete()) {
            foreach ($this->request->post['selected'] as $photo_id) {
                $this->model_sale_photos->deletePhoto($photo_id);
            }
    ...
    protected function getList() {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('sale/photos', 'token=' . $this->session->data['token'] . $url, 'SSL')
        );

        $data['add'] = $this->url->link('sale/photos/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
        $data['delete'] = $this->url->link('sale/photos/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
        ...
        $customer_total = $this->model_sale_photos->getTotalCustomersReport($filter_data);

        $results = $this->model_sale_photos->getCustomersReport($filter_data);

        foreach ($results as $result) {
            if (!$result['approved']) {
                $approve = $this->url->link('sale/photos/approve', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] . $url, 'SSL');
            } else {
                $approve = '';
            }

            $login_info = $this->model_sale_photos->getTotalLoginAttempts($result['email']);
            ...
            $photos = $this->model_sale_photos->getCustomerPhotos($result['c_id'],$result['v_id'],$result['date_created'],$result['added_by']);
            ...
            $data['customers'][] = array(
                ...
                'photos'         => $photos,
                'unlock'         => $unlock,
                'edit'           => $this->url->link('sale/photos/edit', 'token=' . $this->session->data['token'] . '&vehicle_id=' . $result['v_id'] . '$date=' . $result['date_created'] . $url, 'SSL')
            );
        }
        ...
        $data['column_service_types'] = $this->language->get('column_service_types');
        ...
        $data['sort_name'] = $this->url->link('sale/photos', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
        $data['sort_email'] = $this->url->link('sale/photos', 'token=' . $this->session->data['token'] . '&sort=c.email' . $url, 'SSL');
        ...
        $data['sort_date_added'] = $this->url->link('sale/photos', 'token=' . $this->session->data['token'] . '&sort=c.date_added' . $url, 'SSL');
        $data['sort_photo'] = $this->url->link('sale/photos', 'token=' . $this->session->data['token'] . '&sort=c.date_added' . $url, 'SSL');
        ...
        $pagination->url = $this->url->link('sale/photos', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
        ...
        $this->response->setOutput($this->load->view('sale/photos_list.tpl', $data));
    }
    ...
    protected function getForm() {
        ...
        $data['cancel'] = $this->url->link('sale/photos', 'token=' . $this->session->data['token'] . $url, 'SSL');
    ...
    protected function validateDelete() {
        if (!$this->user->hasPermission('modify', 'sale/photos')) {
            $this->error['warning'] = $this->language->get('error_permission');
        }
    ...
    public function autocomplete() {
        ...
        if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_email'])) {
            ...
            $results = $this->model_sale_photos->getCustomers($filter_data);

            foreach ($results as $result) {
                $json[] = array(
                    'customer_id'       => $result['c_id'],
                    'name'              => strip_tags(html_entity_decode($result['c_name'], ENT_QUOTES, 'UTF-8')),
