      ...
      <form action="<?php echo $delete; ?>" method="post" enctype="multipart/form-data" id="form">
        <table class="mytable">
          ...
          <?php if ($adapter) { ?>
            ...
            <tr>
              <td style="text-align: center;"><?php if ($adapter['selected']) { ?>
                <input type="checkbox" name="selected[]" value="<?php echo $adapter['adapter_id']; ?>" checked="checked" />
                <?php } else { ?>
                <input type="checkbox" name="selected[]" value="<?php echo $adapter['adapter_id']; ?>" />
                <?php } ?></td>

                <td style="color: #999;">
                 <?php echo $adapter['adapter_id']; ?>
                </td>
			 ...
			 <td class="left">
              ...
              <div style="float: left; vertical-align: center; padding-top:10px;">
               <?php foreach ($adapter['action'] as $action) { ?>
                 <a href="<?php echo $action['href']; ?>"  style="text-decoration: none; border-bottom: 1px solid;"><?php echo $adapter['adapter_name']; ?></a><br>
                 <?php if ($config_theme == $adapter['adapter_name']) { echo $language->get('theme_status'); } ?>
                <?php
                break;
                } ?>
              ...
              <td class="right"><?php foreach ($adapter['action'] as $action) { ?>
                <a href="<?php echo $action['href']; ?>" class="<?php if($action['text']!='.') echo 'markbuttono'; ?>" style="text-decoration: none; <?php if($action['text']=='.') echo 'float: right;'; ?>"><?php echo $action['text']; ?></a>
                <?php } ?></td>
            </tr>
            <?php } ?>
            <?php } else { ?>
