              ...
              <thead>
                <tr>
                  ...
                  <td class="text-left"><?php if ($sort == 'c.date_added') { ?>
                    ...
                    <?php } ?></td>
                  <td class="text-center "><input type="checkbox" onclick="$('input[name*=\'selected\']').prop('checked', this.checked);" /> <?php echo $column_photo; ?></td>
                  <td class="text-right"><?php echo $column_service_types; ?></td>
                  <td class="text-left"><?php if ($sort == 'c.date_added') { ?>
                            <a href="<?php echo $sort_date_added; ?>" class="<?php echo strtolower($order); ?>"><?php echo $column_added_by; ?></a>
                        <?php } else { ?>
                            <a href="<?php echo $sort_date_added; ?>"><?php echo $column_added_by; ?></a>
                        <?php } ?></td>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td class="text-center">
                          <?php foreach ($customer['photos'] as $photo){ ?>

                                <?php if (in_array($photo['id'], $selected)) { ?>
                                    <input type="checkbox" name="selected[]" value="<?php echo $photo['id']; ?>" checked="checked" />
                                <a class="example-image-link" href="/image/uploads/photos/<?php echo $photo['photo']; ?>"
                                   data-lightbox="example-set-<?php echo $customer['email'] ?>-<?php echo $customer['date_added'] ?>"
                                   data-title="Добавлено: <?php echo $photo['time_created']; ?><br /> Комментарий: <?php echo $photo['caption']; ?>">
                                    <img height="75px" class="example-image" src="/image/uploads/photos/<?php echo $photo['photo']; ?>" alt=""/></a>
                                <?php } else { ?>
                                    <input type="checkbox" name="selected[]" value="<?php echo $photo['id']; ?>" />
                                      <a class="example-image-link" href="/image/uploads/photos/<?php echo $photo['photo']; ?>"
                                         data-lightbox="example-set-<?php echo $customer['email'] ?>-<?php echo $customer['date_added'] ?>"
                                         data-title="Добавлено: <?php echo $photo['time_created']; ?><br /> Комментарий: <?php echo $photo['caption']; ?>">
                                          <img height="75px" class="example-image" src="/image/uploads/photos/<?php echo $photo['photo']; ?>" alt=""/></a>
                    ...
                    <td class="text-left"><?php echo $customer['date_added']; ?></td>
                    <td class="text-left"><?php echo $customer['added_by']; ?></td>
...
$('#button-filter').on('click', function() {
	url = 'index.php?route=sale/photos&token=<?php echo $token; ?>';
...
$('input[name=\'filter_name\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?route=sale/photos/autocomplete&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request),
			dataType: 'json',			
...
$('input[name=\'filter_email\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?route=sale/photos/autocomplete&token=<?php echo $token; ?>&filter_email=' +  encodeURIComponent(request),
			dataType: 'json',			
			success: function(json) {
				response($.map(json, function(item) {
					return {
...
//--></script>
<script src="view/javascript/lightbox.min.js"></script>
</div>
<?php echo $footer; ?> 
